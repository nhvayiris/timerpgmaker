...
//modified this code here too
//added new parameter to make target to constantly perform skill without delay i.e. force skill, no cooldown
  ...
  Skill_Sequencer.prototype.startOnDamageTargetAction = function(action, targets) {
    ...
    switch (cmd) {
	  ...
	  case 'tforceskill': { //add new parameter to notes
        this.targetForceSkill(action, targets); //make it call this function
        break;
      }
    }
  };
  ...
  // this makes it so that if a target was damaged by a skill we can force the first enemy it hit to force cast a spell
  // (in theory could be improved to be used on all targets hit, but not required for the current usage)
  // literally copy pasted the force skill function from the above code, changed the user using the skill from the caster to the hit target.

  Skill_Sequencer.prototype.targetForceSkill = function(action, targets) {
    var id = Number(action[0]);
    var angleOffset = Number(action[1]);
    var radian = targets[0]._radian;
    if (angleOffset) {
      radian += angleOffset * Math.PI / 180;
    }
    var skill = targets[0].forceSkill(id, true);
    skill.radian = radian;
    skill._target = targets[0]._target;
	console.log("XD");
  };
